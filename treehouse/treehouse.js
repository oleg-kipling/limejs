...
goog.require('lime.Director'); // like a Director in Movie
goog.require('lime.Scene'); // make a Scene
...
treehouse.start = function() {
    ...
    // set main game object (frog, walls, badges)
    ...
    var analogStick = new this.lime.Sprite()
                .setSize(350, 327)
                .setPosition(1150, 400)
                .setFill('img/analogStick.png')
                .setAnchorPoint(0,0);

    // button controller object
    ...
    this.lime.scheduleManager.schedule(function(dt) {
        this.checkVictory();
        this.checkMovement(dt); // dt - delta time
    }, treehouse);
...
treehouse.checkMovement = function(dt) {
    if (this.frog.isMoving) {
        ...
        // check if future position hits an obsticle stop movement
